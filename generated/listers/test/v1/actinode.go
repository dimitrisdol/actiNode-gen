...
type ActiNodeLister interface {
	...
	List(selector labels.Selector) (ret []*v1.ActiNode, err error)
...
// List lists all ActiNodes in the indexer.
func (s *actiNodeLister) List(selector labels.Selector) (ret []*v1.ActiNode, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.ActiNode))
	})
	return ret, err
}
...
type ActiNodeNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.ActiNode, error)
...
// List lists all ActiNodes in the indexer for a given namespace.
func (s actiNodeNamespaceLister) List(selector labels.Selector) (ret []*v1.ActiNode, err error) {
...
// Get retrieves the ActiNode from the indexer for a given namespace and name.
func (s actiNodeNamespaceLister) Get(name string) (*v1.ActiNode, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("actinode"), name)
	}
	return obj.(*v1.ActiNode), nil
}
